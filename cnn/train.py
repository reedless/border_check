...
class DataLoader(torch.utils.data.DataLoader):
    ...
    # Converts a list of tuples into a tuple of lists so that it can properly
    # be fed to the model for training
    @staticmethod
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, label_data, image_folder=None, transform=None):
        # CSV file contains: filename, width, height, class, xmin, ymin, xmax,
        # ymax
...
class Model:
    ...
    def __init__(
        ...
        if model_name == self.DEFAULT:  # TODO: update docs when released
            ...
            self._model = torchvision.models.detection.fasterrcnn_mobilenet_v3_large_320_fpn(
                pretrained=pretrained)
        else:
            raise ValueError(
                f'Invalid value {model_name} for model_name. ' +
                f'Please choose between {self.DEFAULT}, {self.MOBILENET}, and {self.MOBILENET_320}.')

        if classes:
            # Get the number of input features for the classifier
            in_features = self._model.roi_heads.box_predictor.cls_score.in_features
            # Replace the pre-trained head with a new one (note: +1 because of
            # the __background__ class)
            self._model.roi_heads.box_predictor = FastRCNNPredictor(
                in_features, len(classes) + 1)
            self._disable_normalize = False
        else:
            raise ValueError(
                'classes must be provided when using a model without a classifier.')
    ...
    # Returns the raw predictions from feeding an image or list of images into
    # the model
    ...
    def predict(self, images):
        ...
        is_single_image = not is_iterable(images)
        ...
        for pred in preds:
            ...
            result = ([self._classes[val]
                      for val in pred['labels']], pred['boxes'], pred['scores'])
    ...
    def fit(
        ...
        # Create an optimizer that uses SGD (stochastic gradient descent) to
        ...
        # Create a learning rate scheduler that decreases learning rate by
        # gamma every lr_step_size epochs
        lr_scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size=lr_step_size, gamma=gamma)

        # Train on the entire dataset for the specified number of times
        # (epochs)
        for epoch in range(epochs):
            ...
            for images, targets in iterable:
                ...
                # loss calculation
                total_loss.backward()
                # Update model parameters from gradients: param -=
                # learning_rate * param.grad
                optimizer.step()
    ...
    def _convert_to_int_labels(self, targets):
        for _, target in enumerate(targets):
            ...
            labels_int_array = [self._int_mapping[class_name]
                                for class_name in labels_array]
